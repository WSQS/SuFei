...
class NarOnnxEngineTest {
    ...
    @Test
    fun testDiProviderResolvesToLoadableModel() {
        // Exercises the real NarTtsModule candidate resolution on-device — regression
        // guard for the scoped-storage EACCES crash: a public /sdcard path must not be
        // preferred over the app-accessible embedded model, and a present-but-unreadable
        // candidate must not abort the engine.
        NarTtsModule.provideNarOnnxEngine(context).use { engine ->
            val audio = engine.synthesize("登鹳雀楼，唐王之涣。白日依山尽。")
            println("DI provider synth: ${audio.size} samples")
            assertTrue("provider engine must synthesize audio, got ${audio.size}", audio.size > 12000)
        }
    }
}
