"""D300 E2E predictor evaluation: predicted-variance vs GT-variance on holdout_20.

For each checkpoint, evaluates two inference modes:
  GT-var (P3):  duration/pitch/energy from teacher (previous eval mode)
  Pred-var (P5): duration/pitch/energy from model's own predictors

Both go through HiFiGAN → ASR → CER. The gap P5−P3 shows how much
degradation the predictor introduces vs teacher-forced variance.

Usage:
  python eval_d300_e2e.py
  python eval_d300_e2e.py --step_labels 24k
  python eval_d300_e2e.py --limit 10
"""
...
def run_gt_var(model, phone_ids, dur_gt_t, pitch_gt, energy_gt_t):
    """Inference with ground-truth variance (teacher forcing)."""
    with torch.no_grad():
...
def run_pred_var(model, phone_ids):
    """Inference with predicted variance (end-to-end, no GT)."""
    with torch.no_grad():
...
def eval_checkpoint(model, manifest, asr, out_dir, label, limit=0):
    ...
    for idx, rec in enumerate(manifest):
        ...
        # Pitch correlation (on voiced frames, aligned by GT duration)
        pitch_gt_expanded = []
        for i, d in enumerate(dur_gt):
            pitch_gt_expanded.extend([f0_norm[i]] * max(d, 0))
        pitch_gt_expanded = np.array(pitch_gt_expanded[:len(pred_pitch)])
...
def main():
    ...
    parser.add_argument(
        "--step_labels",
        nargs="+",
        default=["6k", "12k", "18k", "24k"],
    )
    parser.add_argument("--limit", type=int, default=0)
    a = parser.parse_args()

    step_map = {
        "6k": "D300e2e_step6000_slim.pt",
        "12k": "D300e2e_step12000_slim.pt",
        "18k": "D300e2e_step18000_slim.pt",
        "24k": "D300e2e_step24000_slim.pt",
    }
    ...
    for label in a.step_labels:
        ...
        print(f"\n{'='*70}")
        print(f"D300-E2E HOLDOUT_20 @ {label}")
        print(f"{'='*70}")
        model = load_model(str(ckpt_path))
        out_dir = OUT / f"D300e2e_holdout20_{label}"
        r = eval_checkpoint(model, holdout, asr, out_dir, f"D300e2e_{label}", a.limit)
        all_reports[label] = r

    # Comparison table
    print(f"\n{'='*70}")
    print("D300-E2E PREDICTED-VARIANCE CURVE")
    print(f"{'='*70}")
    print(
