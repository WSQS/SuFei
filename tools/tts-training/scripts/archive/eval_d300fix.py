"""Evaluate D300fix (fixed durations) on holdout_20 with GT variance.

Reuses eval_d300.py logic but points at D300fix checkpoints.
"""
...
def eval_set(model, manifest, asr, out_dir, label):
    ...
    print(f"    P3 mean CER: {s['p3_mean']:.1%}")
...
def main():
    ...
    step_map = {
        "6k": "D300fix_step6000_slim.pt",
        "12k": "D300fix_step12000_slim.pt",
        "18k": "D300fix_step18000_slim.pt",
        "24k": "D300fix_step24000_slim.pt",
    }
    ...
    for label in a.step_labels:
        ...
        print(f"\n{'='*70}")
        print(f"D300FIX HOLDOUT_20 @ {label}")
        print(f"{'='*70}")
        model = load_model(str(ckpt_path))
        out_dir = OUT / f"D300fix_holdout20_{label}"
        r = eval_set(model, holdout, asr, out_dir, f"D300fix_holdout20_{label}")
        all_curves[label] = r

    print(f"\n{'='*70}")
    print("D300FIX HOLDOUT CURVE (fixed durations)")
    print(f"{'='*70}")
    print(f"{'Step':>5} {'delta_mean':>12} {'P3_mean':>8} {'P3=100%':>8} {'L1':>6}")
    print("-" * 45)
    for label in a.step_labels:
        if label not in all_curves or all_curves[label] is None:
            continue
        s = all_curves[label]["summary"]
        print(f"{label:>5} {s['delta_mean']:>+11.1%} {s['p3_mean']:>7.1%} {s['p3_100pct']:>4}/{s['n']} {s['mel_l1_mean']:>.3f}")
