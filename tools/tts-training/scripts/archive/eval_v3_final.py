import json, math, sys, random, numpy as np, torch, soundfile as sf
...
ckpt = torch.load(str(ROOT / 'checkpoints' / 'e2e_v3_step24k.pt'), map_location='cpu', weights_only=False)
model = FastSpeech2(vocab_size=268, dropout=0.0)
model.load_state_dict(ckpt['model'])
model.eval()
print("Loaded e2e_v3_step24k, step=" + str(ckpt.get('step')))
...
OUTDIR = ROOT / 'output' / 'v3_24k_pred'
...
print("\nFor reference: Full E2E v2 pred-all CER = 77.5%%")
