...
ckpt = torch.load(str(ROOT / "checkpoints" / "warmstart_v2_step24k.pt"),
                   map_location="cpu", weights_only=False)
model = FastSpeech2(vocab_size=268, dropout=0.0)
model.load_state_dict(ckpt["model"])
model.eval()
print(f"Loaded warmstart step24k, step={ckpt.get('step', '?')}")
...
print(f"{'='*70}")
print("Warmstart step24k — Train CER (30 samples)")
print(f"{'='*70}")

print("\n--- Condition A: GT dur + GT pitch/energy (decoder quality check) ---")
rA = eval_condition(train_sample, "A", "ws24k_gtvar")

print("\n--- Condition B: pred all (full inference) ---")
rB = eval_condition(train_sample, "B", "ws24k_predvar")
