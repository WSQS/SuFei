...
ckpt = torch.load(str(ROOT / 'checkpoints' / 'mulgate_v5_step24k.pt'), map_location='cpu', weights_only=False)
model = FastSpeech2(vocab_size=268, dropout=0.0)
model.load_state_dict(ckpt['model'])
model.eval()
print("Loaded mulgate_v5_step24k, step=" + str(ckpt.get('step')))
...
OUTDIR = ROOT / 'output' / 'v5_mulgate_24k_pred'
...
print("\n=== mulgate_v5 step24k pred-all CER ===")
