...
ckpt = torch.load(str(ROOT / 'checkpoints' / 'alt_v4_step24k.pt'), map_location='cpu', weights_only=False)
model = FastSpeech2(vocab_size=268, dropout=0.0)
model.load_state_dict(ckpt['model'])
model.eval()
print("Loaded alt_v4_step24k, step=" + str(ckpt.get('step')))
...
OUTDIR = ROOT / 'output' / 'v4_alt_24k_pred'
